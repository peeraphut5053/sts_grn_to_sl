...
if ($printType == 0) { //==== new print 
    if (isset($_POST["coil_ids"])) {
        ...
        for ($t = 0; $t < $ci_n; $t++) {
            ...
            $id = $_POST["coil_ids"][$t];
            $sql = "select Mv_Bc_tag.*, CONVERT(VARCHAR(16), mfg_date, 120)  AS mfg_date 
  , Note = isnull((SELECT TOP 1 CONVERT(NVARCHAR(1000),Note) FROM ReportNotesView R WHERE R.RefRowPointer = grn_hdr_mst.RowPointer and note like '%นำเข้าตาม%'),'')
...
//            $sql = "select *, CONVERT(VARCHAR(16), mfg_date, 120)  AS mfg_date from Mv_Bc_tag where id = '" . $id . "';";
            ...
            $temp->setReplace("{Note}", $rs[1]["Note"]);
        }
    } else {
    ...
    for ($i = 0; $i < count($tag_id); $i++) {
        ...
        $temp->setReplace("{Note}", "" . $ModelTag->note . "");
    }
