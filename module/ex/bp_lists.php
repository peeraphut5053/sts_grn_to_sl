...
$temp->setReplace("{content}", $temp->getTemplate("./template/bp_lists.html"));
...
foreach($result as $indx => $rows ){
    ...
    $lists = $lists . "<tr>"
            ...
            . "<td>" . $StartDate . "</td>"
//            . "<td>" . $EndDate . "</td>"
            . "<td>$CountHC <a href='?bp_view&HeadCode=" . $rows['HeadCode'] . "'><i class='fa fa-search'></i></a></td>"
            . "<td>"
            . "<a href='?bp_edit&HeadCode=" . $rows['HeadCode'] . "'   <i  class='fa fa-edit'></i></a> "
...
if ($Prev_Page) {
    $paging = $paging . " <a class='btn  btn-first' href='?bp_lists&Page=$Prev_Page&txtKeyword=$txtKeyword&advancesearch=$advancesearch&advKeyword=$advKeyword&advCMDate=$advCMDate&advCPDate=$advCPDate'> <<</a> ";
}
$colorActive = "";
for ($i = 1; $i <= $Num_Pages; $i++) {
    if ($i != $Page) {
        $paging = $paging . " <a class='btn  btn-page'  href='?bp_lists&Page=$i&txtKeyword=$txtKeyword&advancesearch=$advancesearch&advKeyword=$advKeyword&advCMDate=$advCMDate&advCPDate=$advCPDate'>$i</a> ";
    } else {
        $paging = $paging . "<b style='background-color:#17a2b8;color:#fff;border:solid 1px #17a2b8;' class='btn  btn-page'> $i </b>";
    }
}
if ($Page != $Num_Pages) {
    $paging = $paging . " <a class='btn  btn-last' href ='?bp_lists&Page=$Next_Page&txtKeyword=$txtKeyword&advancesearch=$advancesearch&advKeyword=$advKeyword&advCMDate=$advCMDate&advCPDate=$advCPDate'> >> </a> ";
}
$paging = $paging . " </div>";
//}
// sqlsrv_close($conn);
...
//$copy = "<a href='?bp_list_cancel' class='btn btn-round'><i class='fa fa-copy'></i></a>" ;
//
//
//
