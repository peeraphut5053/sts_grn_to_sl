...
$temp->setReplace("{content}", $temp->getTemplate("./template/bn_lists.html"));
...
foreach($result as $indx => $rows ){
    ...
    $tDate1 = new DateTime($rows['CmDate']);
    $tDate2 = new DateTime($rows['CpDate']);
    ...
    $lists = $lists . "<tr>"
            ...
            . "<td>" . $CommenceDate . "</td>"
            . "<td>" . $CompleteDate . "</td>"
            . "<td>$CountHC <a href='?bn_view&HeadCode=" . $rows['HeadCode'] . "'><i class='fa fa-search'></i></a></td>"
            . "<td>"
            . "<a href='?bn_edit&HeadCode=" . $rows['HeadCode'] . "'   <i  class='fa fa-edit'></i></a> "
...
if ($Prev_Page) {
    $paging = $paging . " <a class='btn  btn-first' href='?bn_lists&Page=$Prev_Page&txtKeyword=$txtKeyword&advancesearch=$advancesearch&advKeyword=$advKeyword&advCMDate=$advCMDate&advCPDate=$advCPDate'> <<</a> ";
}
$colorActive = "";
for ($i = 1; $i <= $Num_Pages; $i++) {
    if ($i != $Page) {
        $paging = $paging . " <a class='btn  btn-page'  href='?bn_lists&Page=$i&txtKeyword=$txtKeyword&advancesearch=$advancesearch&advKeyword=$advKeyword&advCMDate=$advCMDate&advCPDate=$advCPDate'>$i</a> ";
    } else {
        $paging = $paging . "<b style='background-color:#17a2b8;color:#fff;border:solid 1px #17a2b8;' class='btn  btn-page'> $i </b>";
    }
}
if ($Page != $Num_Pages) {
    $paging = $paging . " <a class='btn  btn-last' href ='?bn_lists&Page=$Next_Page&txtKeyword=$txtKeyword&advancesearch=$advancesearch&advKeyword=$advKeyword&advCMDate=$advCMDate&advCPDate=$advCPDate'> >> </a> ";
}
$paging = $paging . " </div>";
//}
// sqlsrv_close($conn);
...
//$copy = "<a href='?bn_list_cancel' class='btn btn-round'><i class='fa fa-copy'></i></a>" ;
//
//
//
