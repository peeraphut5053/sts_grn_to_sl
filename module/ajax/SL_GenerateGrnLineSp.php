...
for ($i = 0; $i < $x; $i++) {
    ...
    $Lot = new Lot();
    $CheckLot = $Lot->GenNewLotNum($tLot);
    ...
    //============================================//
    //\\/\/\/\\\\\/\\\///\\\ Disable Trigger \\\////\\\\////\\\\////\\\///\\\
    $DisableTriggerIns = sqlsrv_query($conn_sl, "DISABLE Trigger grn_line_mst.grn_line_mstIup ON grn_line_mst ");
    $DisableTriggerDel = sqlsrv_query($conn_sl, "DISABLE Trigger grn_line_mst.grn_line_mstDel ON grn_line_mst ");
    ////\\\\////\\\\////\\\\\/////\\\\\/\\/\/\/\/\/////\\\\////\\\////\\\///\   
    $stmt = sqlsrv_query($conn_sl, $sql);
    if ((!$DisableTriggerIns) || (!$DisableTriggerDel)) {
        ...
        if (!$stmt) {
            ...
            //\\/\/\/\\\\\/\\\///\\ Enable Trigger \\\////\\\\////\\\\////\\\///\\\
            $EnableTriggerIns = sqlsrv_query($conn_sl, "ENABLE Trigger grn_line_mst.grn_line_mstIup ON grn_line_mst ");
            $EnableTriggerDel = sqlsrv_query($conn_sl, "ENABLE Trigger grn_line_mst.grn_line_mstDel ON grn_line_mst ");
            ////\\\\////\\\\////\\\\\/////\\\\\/\\/\/\/\/\/////\\\\////\\\////\\\///\
