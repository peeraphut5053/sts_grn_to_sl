...
class BoatPlanList {
    ...
    public $ItemTag =0;
    public $CoNum ="";
    private $Query1 = "SELECT STS_BoatPlan_List.IdRun,item,qty1,position, STS_BoatPlan_List.HeadCode, itemPosition, itemTag,Position,co_num  "
            . "FROM STS_BoatPlan_List " 
            . "LEFT JOIN STS_BoatPlan_Head ON STS_BoatPlan_List.HeadCode = STS_BoatPlan_Head.HeadCode " 
            . "LEFT JOIN STS_Boat_MT_BoatPosition ON STS_BoatPlan_List.itemPosition = STS_Boat_MT_BoatPosition.IdRun "  
    ...
    function CountRowsWithHeader($HeadCode) {
        $cSql = new SqlSrv();
        $sql0 = $this->Query2. "  WHERE STS_BoatPlan_List.HeadCode= '$HeadCode' ";
        $rs0 = $cSql->SqlQuery($this->StrConn, $sql0);
        return $rs0[1]["CountRecs"];
    }
    ...
    function GetRowsWithHeader() {
        $cSql = new SqlSrv();
        $sql0 = $this->Query1 . "  WHERE STS_BoatPlan_List.HeadCode=  '".$this->_HeadCode."' ";
        $rs0 = $cSql->SqlQuery($this->StrConn, $sql0);
        array_splice($rs0, count($rs0) - 1, 1);
        return $rs0;
    }
