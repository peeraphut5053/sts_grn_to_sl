...
class BoatPlanHead{
    ...
    private $Query1 = "SELECT STS_BoatPlan_Head.HeadId, HeadCode,Boat_Id, Berthed_Id , 
            IsNull(Boat_MV,'N/A') AS Boat_MV,
            IsNull(Boat_LighterNo,'N/A') AS Boat_LighterNo ,
            ...
            ,Cancel
             ...
             LEFT JOIN STS_Boat_MT_Boat ON (STS_BoatPlan_Head.Boat_Id = STS_Boat_MT_Boat.IdRun)
             LEFT JOIN STS_Boat_MT_Harbor ON (STS_BoatPlan_Head.Berthed_Id = STS_Boat_MT_Harbor.IdRun )  ";
...
//        $this->EndDate = $EndDate;
    ...
    function GetRowsWithCondAndLimit($sWhere, $limitStart, $limitEnd) {
        $cSql = new SqlSrv();
        $sql = "WITH QR_HEAD AS ( SELECT  ROW_NUMBER() OVER (ORDER BY HeadCode) AS [Row_No], * FROM  STS_BoatPlan_Head  WHERE  ( Cancel=" . $this->_Cancel . " ) ) "
                . " SELECT HeadId , HeadCode, Boat_Id,Berthed_Id ,
                    NULLIF(Boat_MV,'N/A') AS Boat_MV,
                    NULLIF(Boat_LighterNo,'N/A') AS Boat_LighterNo,
                    NULLIF(HB_Name,'N/A') AS  HB_Name,
                    CONVERT(nvarchar, StartDate) as StartDate, 
                    CONVERT(nvarchar,EndDate) AS EndDate
                FROM QR_HEAD 
                LEFT JOIN STS_Boat_MT_Boat ON QR_HEAD.Boat_Id = STS_Boat_MT_Boat.IdRun 
                LEFT JOIN STS_Boat_MT_Harbor ON QR_HEAD.Berthed_Id = STS_Boat_MT_Harbor.IdRun"
                . " WHERE $sWhere AND  [Row_No] between $limitStart and $limitEnd   ";
